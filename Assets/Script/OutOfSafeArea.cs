...
public class OutOfSafeArea : MonoBehaviour
{
    #region Simulations
    /// <summary>
    ...
    {
        LastSafeArea = r;
        float height = (Screen.height - LastSafeArea.y - LastSafeArea.height);
        var _ups = GameObject.Find("UpS").GetComponent<RawImage>();
        var _downs = GameObject.Find("DownS").GetComponent<RawImage>();
        _ups.GetComponent<RectTransform>().sizeDelta = new Vector2(Screen.width, height);
        _downs.GetComponent<RectTransform>().sizeDelta = new Vector2(Screen.width, height);
